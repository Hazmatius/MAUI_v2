...
function selected_points_listbox_Callback(hObject, eventdata, handles)
    if strcmp(get(gcf,'selectiontype'),'open')
        ...
        if ~isempty(point_name)
            point_path = pipeline_data.points.namesToPaths(point_name);
            pipeline_data.points.remove('name', point_name);
            pipeline_data.points.add({point_path});
            pipeline_data.points.setLabels(pipeline_data.points.pathsToPoints(point_path).labels);
            set(handles.background_channel_menu, 'string', pipeline_data.points.labels());
            if isempty(pipeline_data.points.bgRmParams)
                pipeline_data.points.initBgRmParams()
            end
            % point = pipeline_data.points.pathsToPoints(point_path);
        end
    end
